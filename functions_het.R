...
calculate_coverage_rate <- function(lower_bounds, upper_bounds, true_value, REP) {
  ...
  coverage_rate <- sum(contains_true_value) / REP * 100
...
reg1 = function(Y, R, M, X1, X2, X3, C, b.m = NULL, b.y = NULL, B, data){
 ...
 # weighting
    ...
    moClass1 <- buildModelObj(model = ~R + C + X1 + X2 + X3 +U,
                              solver.method = "rpart",
...
SE_delta_IIE = sd(delta_IIE.boot)
SE_zeta_IIE = sd(zeta_IIE.boot)
...
reg2 = function(Y, R, M, X1, X2, X3, C, b.m = NULL, b.y = NULL, B, data){
  ...
  est1 = function(data){  
    ...
    #Q-Learn
    # moMain <- buildModelObj(model = ~R + M+C + X1 + X2  ,
    #                         solver.method = 'lm')
    # moCont <- buildModelObj(model = ~X1 + X2,
    #                         solver.method = 'lm')
    # fitFS <- qLearn(moMain=moMain, moCont= moCont, data=data, response=data$Y, txName='M')
...
genU1 = function(Y, R, M, X1, X2, X3, C, eb.y, eb.yu,b.m, p.u, Iternum, data) {
  ...
  for (iter in 1:Iternum) {
    # Outcome model
    l.y = lm(as.formula(paste("Y", "~", "R", "+", "X1","*" , "M", "+", paste("C",  "X2", "X3", collapse = "+", sep = "+"))),
            offset = eb.y * data$U + eb.yu * data$M*data$U, data = data)
    coef.y = c(l.y$coef, U = eb.y, M_U = eb.yu)
    ...
    mean.yu1 = cbind(model.matrix(l.y), U = 1, M_U =model.matrix(l.y)[,"M"]) %*% coef.y
    sd.yu1 = sd.y; #sd(l.y$residuals[data$U==1])
    mean.yu0 = cbind(model.matrix(l.y), U = 0, M_U =0) %*% coef.y
    sd.yu0 = sd.y; #sd(l.y$residuals[data$U==0])
