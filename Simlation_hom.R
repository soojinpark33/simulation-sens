...
for(nn in 1:3){
  ...
  b.m <-b.y <-1.5 # change to 1 or 0.5
  beta_m <- 1
  ...
  #Below is a modified code to use 'mclapply' function. Outer-loop is paralleled.
  ...
  simulate_one_ind <- function(j, popdata, nsample, nsim, Y, R, M, X1, X2, X3, C, b.y, b.m, p.u, Iternum, B) {
    ...
    for(k in 1:nsim) {
      data$U = genU1(Y=Y, R=R, M=M, X1=X1, X2=X2, X3=X3, C=C, b.y=b.y, b.m=b.m, p.u=p.u, Iternum = Iternum, data= data)$new_U
  ...
  #results_ind <- mclapply(1:REP, simulate_one_ind, popdata = popdata, nsample = nsample, nsim = nsim, Y = Y, R = R, 
  #                        M = M, X = X, C = C, b.y = b.y, b.m = b.m, p.u = p.u, Iternum = Iternum, B = B,
  #                        mc.cores = num_cores)
  ...
  rm(popdata)
  save.image(paste0("indivdualized_hom", nsample, ".1.5.RData"))
